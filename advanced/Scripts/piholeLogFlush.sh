...
if [[ "$@" != *"quiet"* ]]; then
    echo -ne "  ${INFO} Flushing /tmp/pihole-logs/var/log/pihole.log ..."
fi
if [[ "$@" == *"once"* ]]; then
    # Nightly logrotation
    if command -v /usr/sbin/logrotate >/dev/null; then
        ...
        cp /tmp/pihole-logs/var/log/pihole.log /tmp/pihole-logs/var/log/pihole.log.1
        echo " " > /tmp/pihole-logs/var/log/pihole.log
    fi
else
    # Manual flushing
    if command -v /usr/sbin/logrotate >/dev/null; then
        ...
        echo " " > /tmp/pihole-logs/var/log/pihole.log
        if [ -f /tmp/pihole-logs/var/log/pihole.log.1 ]; then
            echo " " > /tmp/pihole-logs/var/log/pihole.log.1
        fi
    fi
...
if [[ "$@" != *"quiet"* ]]; then
    echo -e "${OVER}  ${TICK} Flushed /tmp/pihole-logs/var/log/pihole.log"
